...
{	float buf [channels * BLOCK_SIZE] ;
	int k, m, readcount ;
	int sampleHoldCount[channels],lastSampleHold[channels];
	for (m = 0 ; m < channels ; m++)
    {
        sampleHoldCount[m]=0;
        lastSampleHold[m]=0;
    }
	float currSample[channels],precSample[channels];
	long currSampleIndex =0;
    double FloatEpsEquality = 0.00006;
			...
			for (m = 0 ; m < channels ; m++)
			{
					...
					if (k==0)
                    {
                      ...
                      if (currSampleIndex==1)
                        ...
                        {
                        precSample[m] = currSample[m];
                        }
                    }
                    else
                    {
                        currSample[m] = buf [k * channels + m]; /*Current Sample Value */
                        precSample[m] = 	buf [(k-1) * channels + m];/*Precedent Sample Value*/
                        /*Is the current sample value equal to the precedent sample value ?*/
                        /*Exclude digital mute: >0.1*/
                        /*Exclude Fullscale saturation < 1-1- 0.00006 */
                        /*printf("channel: %d, Echantillon: %ld, sampleHoldCount %d, precSample %0.7f,  currSample %0.7f\n",m, currSampleIndex,sampleHoldCount,precSample,currSample);*/
                        if(currSample[m] >= (precSample[m] - FloatEpsEquality)  && currSample[m] <= (precSample[m] + FloatEpsEquality) &&  currSample[m] >= 0.1  && currSample[m] <= (1- FloatEpsEquality))
                        {
                            sampleHoldCount[m]++;
                            /*printf("Sample hold: channel: %d, Echantillon: %ld, sampleHoldCount %d\n",m, currSampleIndex,sampleHoldCount[m]);*/
                            if(sampleHoldCount[m]>=sampleHoldTresh)
                                {
                                printf("%0.5f	%0.5f	Suspicious Sample Hold of  %d samples, channel %d \n",(float)(currSampleIndex) / sampleRate,(float)(currSampleIndex) / sampleRate,sampleHoldCount[m],m);
                                if(k-lastSampleHold[m]>=mergeTimeHolds)
                                    {

                                    lastSampleHold[m]=k;
                                    sampleHoldCount[m]=0;
                                    /*printf("mergeTimeHolds channel: %d, precSample %0.7f,  currSample %0.7f\n",m, precSample[m],currSample[m]);*/
                                    /*printf("sampleHoldTresh %d\n",sampleHoldTresh);*/
                                    }
                                else
                                    {
                                    /*printf("mergeTimeHolds 1 Echantillon: %d, sampleHoldCount %d\n",k,sampleHoldCount[m]);*/
                                    sampleHoldCount[m]=0;
